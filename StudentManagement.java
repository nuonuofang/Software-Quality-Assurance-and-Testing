...
public class StudentManagement extends JFrame implements ActionListener {
	...
	JTextArea findArea; 
	StudentPanel findInnerPanel; // 查找数据
	JLabel findInputLbl;
	...
	StudentPanel deleteInnerPanel;//删除数据
	...
	public void serGUIComponent() {
		...
		// 定义查找面板
				...
				findInputLbl = new JLabel("查找姓名：");//定义一个标签
				findInputText = new JTextField(10);//定义一个输入框
				findInputText.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						// 将更新面板的显示内容清除
						viewARecord(findInputText.getText(), findInnerPanel);
						//findInputText.setText("");//不清除
					}
				});
				
				
				findBtn1 = new JButton("查找");
				findBtn2 = new JButton("修改");
				findBtn3 = new JButton("删除");
				...
				findPanel.add(findInnerPanel);
				dbTabPane.addTab("查找数据", findPanel);
				dbTabPane.add("查找数据", findPanel);
		...
		// 定义查找面板
		// findBtn1=new JButton("按姓名查找");

		// 定义更新面板
		updateInnerPanel = new StudentPanel();
		updateInputLbl = new JLabel("已知姓名：");//定义一个标签
		updateInputText = new JTextField(10);//定义一个输入框
		...
		inputNameLabel = new JLabel("已知姓名：");
	...
	public void connection() { // 建立连接数据库
		try {
			Class.forName("com.mysql.jdbc.Driver"); // JDBC-ODBC桥接器
	...
	public void inputRecords() { // 录入学生数据
		...
		try {
			connection();
			String InsSQL;
			InsSQL = "INSERT INTO student(学号,姓名,性别,出生年月,家庭住址,联系电话)" + "VALUES("
					+ "'" + id + "'," + "'" + name + "'," + "'" + gender + "',"
					+ "'" + birDate + "'," + "'" + address + "'," + "'" + tel
					+ "')";
			stmt.execute(InsSQL);
	...
	public void viewARecord(String name, StudentPanel p) {
		try {
			connection();
			String viewSQL = "SELECT * From student where 姓名='" + name + "'";
			ResultSet rs = stmt.executeQuery(viewSQL);
			if (rs.next()) {
				p.setName(rs.getString("姓名"));
				p.setId(rs.getString("学号"));
				p.setGender(rs.getString("性别"));
				p.setBirDate(rs.getString("出生年月"));
	...
	public void viewRecords() {
		try {
			...
			connection();
			ResultSet rs = stmt
					.executeQuery("SELECT * From student order by 学号");// 按学号递增排序
	...
	public void findRecords(String name) {
		try {
			...
			connection();
			ResultSet rs = stmt
					.executeQuery("SELECT * From student where 姓名='" + name +"'");// 按学号递增排序
			ResultSetMetaData rsMeta = rs.getMetaData();
			int nums = rsMeta.getColumnCount();
			// 获得字段名称
			for (int i = 1; i <= nums; i++) {
	...
	public void updateRecord(String name) { // 修改姓名为name学生的记录
		String updateSQL = "update student set 姓名 =?," + "性别=?," + "出生年月=?,"
				+ "家庭住址=?," + "联系电话=?" + "" + "where 姓名='" + name + "'";
	...
	public void deleteRecord(String name) {
		String delStr = "delete from student where 姓名='" + name + "'";
		try {
			connection();
